...
def test(ML, key):
  ...
  print ("in maturity level " + str(ML) + ", namespace '*' [i.e. only matches config.s that are present in all namespaces at least in this ML]:\n\t " + "for key = ''" + key + "'': textual result = ''" + str(Configuration_Master_client.get___config_as_string___or_NoneType(key=key, ML=ML)) + "'', result = " + str(Configuration_Master_client.get___config_as_Boolean___or_NoneType(key=key, ML=ML)))
  print ()
